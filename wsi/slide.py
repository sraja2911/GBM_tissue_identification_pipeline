...
def open_slide(filename):
  ...
  try:
    ...
    print("got openslide open: " + filename)
  except OpenSlideError:
    slide = None
    print("openslide error: "+ slide)
...
def assign_slide_number():
  """
  Assigns slide path to all the physical WSI training slide file.
  Example:
    5 -> ../data/training_slides/TCGA-005-<fname>.svs
  Args:
    slide_number: src_dir.
  """
  #src_dir = SRC_TRAIN_DIR
  count = 1
  img_data_list=[]
  #img_data_list=os.listdir(src_dir)
  img_data_list=os.listdir(SRC_TRAIN_DIR)
  src_abs_path = SRC_TRAIN_DIR +"/"

  for dataset in img_data_list:
      padded_sl_num = str(count).zfill(3)
      new_filename = TRAIN_PREFIX + padded_sl_num + "-" + str(dataset)
      shutil.move(src_abs_path + dataset, src_abs_path + new_filename) 
      count +=1     
  return 
...
def get_training_slide_path(slide_number):
  ...
  """
  img_list=[]
  img_list=os.listdir(SRC_TRAIN_DIR)
  #filename = img_list[slide_number]
  filename = img_list[slide_number-1]
  slide_abspath = SRC_TRAIN_DIR + "/"
  slide_filepath = slide_abspath + filename
...
def slide_to_scaled_pil_image(slide_number):
  ...
  #slide_name = slide_filepath
  #slide_abspath= SRC_TRAIN_DIR + "/" + slide_filepath 
